...
{
    public class ChaseableLaunchOperation : LaunchOperation
    {
        private readonly Dictionary<string, ActionLaunchConfiguration> configurations;

        public ChaseableLaunchOperation(string actionKey, params ActionLaunchConfiguration[] configurations) : base(LaunchSource.Chaseable)
        {
        ...
        public override async Task Run(LaunchActivatedEventArgs args, Frame frame)
        {
            ...
            {
                var notification = args.TileActivatedInfo.RecentlyShownNotifications[0];
                var query = QueryString.Parse(notification.Arguments);
